...
fails the build if alt text is missing or left as the `IMAGE_ALT` placeholder, which is
worse than nothing for anyone using a screen reader.
...
**Don't describe how a live product is built.** For anything still running, write what it
does for the person using it, not the internals: no framework names, no pipeline
architecture, no latency or performance tricks. Capabilities, scale, and outcomes are fair
game; implementation is not. This page is public and permanent, so the bar is higher than
a resume's.

**Add press coverage to a job** with `links`, whenever it exists:
...
Don't add scraped article thumbnails. A publication's own preview image is usually a photo
of someone else, and putting it on your page implies they're you or your team.
...
An `href` turns the label into a link so the claim can be checked. Add one wherever the
proof is public.
...
**Open with the problem, not the stack.** The problem a reader recognises earns a read;
a list of technologies doesn't. A question works well for research write-ups.
...
Don't generate a picture to fill a gap — that's decoration pretending to be evidence. A
project with no `image` just renders as text, which is fine.
